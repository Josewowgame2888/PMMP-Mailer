...
class GoogleMail {
  ...
  private function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|ReplyTo)$/', $kind)) {
      GoogleError::logger('Invalid recipient array: ' . $kind);
      return false;
    }
    ...
    if (!self::ValidateAddress($address)) {
      ...
      GoogleError::logger($this->Lang('invalid_address').': '.$address);
      return false;
    }
    if ($kind != 'ReplyTo') {
  ...
  public function Send() {
    try {
      ...
      GoogleError::logger($e->getMessage());
      return false;
    }
  }
  ...
  protected function SmtpSend($header, $body) {
    require_once $this->PluginDir . 'GoogleSMTP.php';
    $bad_rcpt = array();
    if(!$this->SmtpConnect()) {
